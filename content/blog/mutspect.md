...
This idea is well-established in oncology. Cancer epidemiolgy studies have shown that a handful of genes, most commonly the human p53 gene, show cancer-specific patterns of mutation. Further, certain mutagens are associated with certain mutation types or patterns. For example, in Pfeifer & Besaratinia's 2009 review[^1] on the subject, they summarised findings from various studies on mouse models containing human p53 genes (_Hupki_ mouse models).  
...
Mutational spectra in the (human) p53 gene in _Hupki_ mice were enriched in G>T mutations when exposed to a tobacco-derived carcinogen (panel A above) and the overall pattern of mutation resembled that from lung-cancer patients who smoked. Similarly, exposure to a plant extract thought to be lead to urothelial cancers produced a elevated A>T mutations (panel B above), a hallmark for these cancers, and exposure to a particulate air pollutant produced a mutational pattern identical to that seen in other systems exposed to that pollutant (panel C above).  

### Mutational Spectra in Microbes

In a 2021 paper[^2], Chris Ruis and colleagues observed that _Mycobacterium abscessus_ isolated from the lungs of cystic fibrosis sufferers showed spectra of mutations that were distinct from environmental isolates (Fig 2 below). In a subsequent work, Ruis, et al., calculated mutational spectra for data from a range of microbial samples, and attempted to associate them with specific mutagenic contexts. They found that distinct patterns of variation were associated with specific DNA-repair defects and ecological niches[^3]. To resonctruct these mutational spectra, Chris wrote a tool called [MutTui](https://chrisruis.github.io/MutTui/#/), which they later used to show that mutation patterns in SARS-CoV2 were different between lineages that replicated in the upper and lower respiratory tracts[^4].  
...
Given the evidence for the correlation between the ecological niche a microbe occupies and the patterns of mutation it accumulates, we reasoned that mutational spectra might be similarly correlated to epidemiological factors in _Streptocuccus pneumoniae_, a genetically diverse, pathogenic bacterium that can cause pneumonia and meningitis in individuals with weaker or weakened immune systems, such as children or the elderly, and immunodeficient or immunosuppressed individuals. One epidemiological feature of interest was carriage duration (CD), the length of time that pneumococcus resides in a given individual before it is transmitted to another. A 2017 study led by John[^5] estimated CD for samples from the Maela dataset[^6], a densely sampled, longitudinal dataset derived from a camp for displaced persons in Thailand, and showed that CD is heritable, and variability in CD is attributable to the pathogen's genotype.  

We used CD data estimated from this study, and combined it with information about sample lineage derived from the [Global Pneumococcal Sequencing (GPS)](https://www.pneumogen.net/gps/) project. Briefly, the GPS project uses [PopPUNK](https://github.com/bacpop/PopPUNK), which uses a kmer-based approach to calculate similarity between the input samples and assigns them to lineages, or clusters (GPS clusters, henceforth referred to as GPSCs).  

{{< figure src="/images/mutspect/fig3.png" title="Figure 3: Our MutTui workflow" >}}  

Our workflow involved associating precalculated GPSC assignments to each Maela sample, and using MutTui to reconstruct mutational spectra for the 33 GPSCs that we retained, after discarding those GPSCs that were represented by less than 20 samples in the Malea dataset.  
...
As noted earlier, MutTui produces mutation spectra that account for mutation context, i.e. the nucleotides on either side of the variant site. We removed this context, summing all mutations of the same type, and ran UMAP on this simpler dataset, obtaining similar results. We also coloured these UMAP projections with a range of metadata: we converted categorical and binary data into 1s and 0s, and plotted averages for a quick, overall view. For examples, for drug susceptibility, we assigned 1 to susceptible samples and 0 to both resistant ones and ones that had intermediate phenotypes; averaging over such data would yeild the fraction of susceptible samples in each cluster. However no clear patterns emerged for any of the metadata we tested.

Next, we performed a risk calculation for each substitution type. Briefly, we calculated a True/False value for:  
